...
void* allocate_vector(int size, int m){
  ...
  if ((a = (void *)malloc(m * size)) == NULL){
     fprintf(stderr, "Error : Memory does not enough! in vector \n");
     exit(1);
  }
  return a;
}
...
void** allocate_matrix(int size, int m, int n){
  ...
  if ((aa = (void **)malloc(m * sizeof(void*))) == NULL){
     fprintf(stderr, "Error : Memory does not enough! aa in matrix \n");
     exit(1);
  }
  if ((aa[0]=(void *)malloc(m * n * size)) == NULL){
     fprintf(stderr, "Error : Memory does not enough! in matrix \n");
     exit(1);
  }
