...
int main(int argc, char **argv){
  ...
  int itmax, iout, nnod, nelem, ibc, iflag1 = 0, iflag2 = 0;
  double uu, vv, skx, sky, dt;

  int *icomrnk, *icomnod, **ncomnod, *localnod, *iglnbd, *isendreq, *irecvreq;
  double **sendbuf, **recvbuf;

  FILE *fp1, *fp2, *fp3;
  int j, myid, numprocs, ifilerank, icom;
  ...
  for(int irank = 0; irank < myid+1; irank++) fscanf(fp2, "%d %d %d", &ifilerank, &nnod, &nelem);
  ...
  for(int irank = 0; irank < myid+1; irank++){
    for(int i = 0; i < nnod; i++) fscanf(fp2, "%d %lf %lf", &j, &xx[i], &yy[i]);
    for(int i = 0; i < nelem; i++) fscanf(fp2, "%d %lf %lf %lf", &j, &nc[0][i], &nc[1][i], &nc[2][i]);
    ...
    if(ibc != 0){
      ...
      for(int i = 0; i < ibc; i++) fscanf(fp2, "%d %d %lf", &j, &nbc[i], &fbc[i]);
      for(int i = 0; i < ibc; i++) nbc[i] -= 1;
      //for(int i = 0; i < *ibc; i++) fprintf(stderr, "nbc[%d] = %d\n", i, nbc[i]);
    }
    fscanf(fp2, "%d", &icom);
    if(iflag2 == 0){
      icomrnk = (int*)allocate_vector(sizeof(int), icom);
      icomnod = (int*)allocate_vector(sizeof(int), icom);
      ncomnod = (int**)allocate_matrix(sizeof(int), icom, icom);
      iflag2++;
    }
    for(int i = 0; i < icom; i++) fscanf(fp2, "%d", &icomrnk[i]);
    for(int i = 0; i < icom; i++) fscanf(fp2, "%d", &icomnod[i]);
    for(int i = 0; i < icom; i++){
      for(int j = 0; j < icomnod[i]; j++) fscanf(fp2, "%d", &ncomnod[i][j]);
    }
    for(int i = 0; i < icom; i++){
      for(int j = 0; j < icomnod[i]; j++) ncomnod[i][j] -= 1;
    }
    for(int i = 0; i < icom; i++){
      icomrnk[i] -= 1;
      icomnod[i] -= 1;
    }
    if(ifilerank == myid+1) break;
  }
